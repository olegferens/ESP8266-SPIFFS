...
String GenPageStyleCSS(bool zero)
{
  String page_style_css;
  if (!zero)
  {
    page_style_css = "<title>I have been hack</title>\n"
                      "<font face=\"verdana\">\n"
                      "<style type=\"text/css\">\n"
                      "    #submit {\n"
...
// Reset html page
String reset_html = GenPageStyleCSS(false) +
                    menu_html +
...
// About html page
String about_html = GenPageStyleCSS(false) +
                    menu_html +
...
// No args
// Return 1 String
String GenSettingsHTML(bool main)
{
  String settings_html;
  if (!main)
  {
    settings_html = settings_html + 
                      "<title>I have been hack</title>\n"
                      "<font face=\"verdana\">\n"
                      "<style type=\"text/css\">\n"
                      "    #submit {\n"
                      "        background-color: " + color1 + ";\n"
    ...
    settings_html = GenPageStyleCSS(false) +
                    menu_html +
...
void SettingsESP()
{
  ...
  //String settings_html = GenPageStyleCSS(false);
  //Serial.print("settings_html.length() -> ");
  //Serial.println(settings_html.length());
  
  String settings_html = GenSettingsHTML(false);
  Serial.print("settings_html.length() -> ");
  Serial.println(settings_html.length());
  
  server.send(200, "text/html", settings_html);
  
  //Serial.print("GenPageStyleCSS().length() -> ");
  //Serial.println(GenPageStyleCSS(false).length());
  
  //settings_html = GenPageStyleCSS(false);
  //Serial.print("Zeroed settings_html -> ");
  //Serial.println(settings_html.length());
...
void HandleClient()
{
  server.send(200, "text/html", GenSettingsHTML(true));
}
